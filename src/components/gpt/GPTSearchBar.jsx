    ...
    <div className='flex justify-center bg-black bg-opacity-75 w-1/2  p-3 mx-auto rounded-md'>
      ...
      >
        <input
          ...
          placeholder={language[lang].gptPlaceHolder?language[lang].gptPlaceHolder:""} />
        <button
          onClick={() => handlegeminiAI(searchRef.current.value)}
          className='col-span-3 bg-red-600 rounded-md text-white text-xl font-semibold'>{language[lang].search?language[lang].search:""}</button>
      </form>
    </div>
  )
}
